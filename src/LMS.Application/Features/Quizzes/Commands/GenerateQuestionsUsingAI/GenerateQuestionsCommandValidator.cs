...
public class GenerateQuestionsCommandValidator : AbstractValidator<GenerateQuestionsCommand>
{
    ...
    {
        RuleFor(x => x.FileIds)
            .NotNull().WithMessage("FileIds can't be null.")
            .NotEmpty().WithMessage("at least one file must be selected.");
        ...
        RuleFor(x => x.QuestionTypeCounts)
            ...
            .WithMessage("Question types must be MCQ, TrueFalse, or Written.");

        RuleFor(x => x.QuestionDifficultyPercents)
            .NotNull().WithMessage("Difficulty levels can't be null.")
            .NotEmpty().WithMessage("Difficulty levels must be specified.")
            .Must(x => x.Keys.All(k =>
